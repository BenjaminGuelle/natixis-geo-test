...
export class GeoApiService implements GeoRepositoryInterface {
  ...
  searchRegions(query: string): Observable<RegionModel[]> {
    ...
    return this.http.get<RegionDto[]>(`${this.#baseUrl}/regions`)
      .pipe(
  ...
  getDepartmentsByRegion(regionCode: string): Observable<DepartmentModel[]> {
    return this.http.get<DepartmentDto[]>(`${this.#baseUrl}/regions/${regionCode}/departements`)
      .pipe(
  ...
  getDepartmentByCode(code: string): Observable<DepartmentModel> {
    return this.http.get<DepartmentDto>(`${this.#baseUrl}/departements/${code}`)
      .pipe(
        map((dto: DepartmentDto) => ({
          code: dto.code,
          name: dto.nom,
          regionCode: dto.codeRegion,
        })),
  ...
  getMunicipalitiesByDepartment(departmentCode: string): Observable<MunicipalityModel[]> {
    return this.http.get<MunicipalityDto[]>(`${this.#baseUrl}/departements/${departmentCode}/communes`)
      .pipe(
